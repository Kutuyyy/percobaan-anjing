...
local function getScrapperTarget()
    if ScrapperTargetFarm and ScrapperTargetFarm.Parent then return ScrapperTargetFarm end
...
---------------------------------------------------------
-- GLOBAL SHUTDOWN FUNCTION (DIPINDAHKAN KE SINI SEBELUM UI)
---------------------------------------------------------
local function globalShutdown()
    scriptDisabled = true
    print("[SHUTDOWN] Mematikan semua sistem...")
    
    -- Hentikan semua loops dengan increment ID
    CookLoopId += 1
    ScrapLoopId += 1
...
---------------------------------------------------------
-- WEBHOOK HELPERS (from contekan.lua - LENGKAP)
---------------------------------------------------------
...
local function initRemoteEvents()
    ...
    -- Remote untuk Bring Item System (ASLI)
    RequestStartDragging = re:WaitForChild("RequestStartDraggingItem")
    RequestStopDragging = re:WaitForChild("StopDraggingItem")
    
    CollectCoinRemote = re:FindFirstChild("RequestCollectCoints") or re:FindFirstChild("CollectCoin")
    ConsumeItemRemote = re:FindFirstChild("RequestConsumeItem") or re:FindFirstChild("ConsumeItem")
    ...
    -- Remote untuk GodMode
    local damageRemote = re:FindFirstChild("DamagePlayer")
    
    notifyUI("Remote Events", "Semua remote ditemukan!", 3, "radio")
    return true
end
...
mainTab:Toggle({
    ...
    Callback = function(v)
        GodmodeEnabled = v
        WindUI:Notify({Title="GodMode", Content=v and "ON" or "OFF", Duration=4})
    end
})
...
mainTab:Button({
    ...
    Icon = "power",
    Variant = "Destructive",
    Callback = globalShutdown  -- LANGSUNG PANGGIL FUNGSI, TANPA DUPLIKAT KODE
})

---------------------------------------------------------
-- LOCAL PLAYER TAB CONTENT (FULL - SAMA DENGAN CONTEKAN.LUA)
---------------------------------------------------------
...
FarmTab:Toggle({
    ...
    Callback = function(state)
        if scriptDisabled then return end
        if state then
            local ok = ensureScrapperTargetFarm()  -- Ganti di sini
            if not ok then
                ScrapEnabled = false
                WindUI:Notify({
                    Title = "Auto Scrapper",
...
FarmTab:Toggle({
    ...
    Callback = function(state)
        if scriptDisabled then return end
        if state then
            local ok = ensureScrapperTargetFarm()  -- GANTI INI
            if not ok then
                ScrapEnabled = false
                WindUI:Notify({
                    Title = "Auto Scrapper",
...
FarmTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then 
            ...
            WindUI:Notify({
                Title = "Chop Aura",
                Content = "Chop Aura diaktifkan! Cache dibangun.",
                Duration = 4
            })
        else 
            TreeCache = {}
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- For Papi Dimz system
    humanoid = char:WaitForChild("Humanoid")
    rootPart = char:WaitForChild("HumanoidRootPart")
    defaultWalkSpeed = humanoid.WalkSpeed
    
    -- For Bring Item system
    Character = char
...
---------------------------------------------------------
-- WEBHOOK TAB CONTENT (FULL - SAMA DENGAN CONTEKAN.LUA)
---------------------------------------------------------
WebhookTab:Input({ 
    ...
    Callback = function(txt) 
        ...
        if t ~= "" then 
            WebhookURL = t; 
            WindUI:Notify({Title="Webhook", Content="URL disimpan.", Duration=3, Icon="link"}); 
            print("WebhookURL set:", WebhookURL) 
        end 
    end 
})

WebhookTab:Input({ 
    Title = "Webhook Username (opsional)", 
    Icon = "user", 
    ...
    Callback = function(txt) 
        ...
        WindUI:Notify({Title="Webhook", Content="Username disimpan: " .. tostring(WebhookUsername), Duration=3, Icon="user"}) 
    end 
})
...
WebhookTab:Toggle({ 
    ...
    Callback = function(state) 
        ...
        WindUI:Notify({Title="Webhook", Content=state and "Webhook diaktifkan." or "Webhook dimatikan.", Duration=3, Icon=state and "check-circle-2" or "x-circle"}) 
    end 
})
...
WebhookTab:Button({ 
    ...
    Callback = function()
        ...
        local payload = { 
            ...
            embeds = {{
                title = "🧪 TEST - Webhook Aktif " .. tostring(WebhookUsername), 
                description = ("**Webhook Aktif %s**\n\n**Progress:** `%s`\n\n**Pemain Aktif:**\n%s"):format(tostring(WebhookUsername), tostring(currentDayCached), namesToVerticalList(names)), 
                color = 0x2ECC71, 
                footer = { text = "Test sent: " .. os.date("%Y-%m-%d %H:%M:%S") }
            }}
        }
        local ok, msg = sendWebhookPayload(payload)
        if ok then 
            WindUI:Notify({Title="Webhook Test", Content="Terkirim: " .. tostring(msg), Duration=5, Icon="check-circle-2"}); 
            print("Webhook Test success:", msg)
        else 
            WindUI:Notify({Title="Webhook Test Failed", Content=tostring(msg), Duration=8, Icon="alert-triangle"}); 
            warn("Webhook Test failed:", msg) 
        end
    end
})
...
task.spawn(function()
    initRemoteEvents()
    -- Hook DayDisplay untuk webhook (VERSI LENGKAP)
    tryHookDayDisplay()
end)

-- 2. Auto-sacrifice loop safety
task.spawn(function()
    while not scriptDisabled do
        if AutoSacEnabled and lavaFound and ItemsFolder then
            for _, obj in ipairs(ItemsFolder:GetChildren()) do
...
---------------------------------------------------------
-- Assign ke tombol shutdown di Main Tab
-- (Tombol sudah ada di Main Tab, pastikan callback-nya memanggil globalShutdown)

---------------------------------------------------------
-- LOADED NOTIFICATION (FINAL)
---------------------------------------------------------
task.wait(1.5) -- Beri waktu untuk inisialisasi
WindUI:Notify({
    Title = "✅ Papi Dimz Ultimate Hub Siap!",
    Content = "TP Walk & Webhook System diupdate ke versi lengkap.",
    Icon = "check-circle",
    Duration = 8
})
print([[
[PapiDimz] MERGE COMPLETE - ENHANCED
===========================
- TP Walk System: ADDED ✓
- Webhook System: FULL ✓
