...
local TreeCache = {}
local SelectedTreeCategory = "Small Tree" -- Default category
...
local function buildTreeCache()
    ...
    -- Ambil daftar nama pohon berdasarkan kategori yang dipilih
    local targetTreeNames = TreeCategories[SelectedTreeCategory]
    if not targetTreeNames then
        warn("[ChopAura] Kategori pohon tidak ditemukan:", SelectedTreeCategory)
        return
    end
    ...
    local function scanFolder(folder)
        ...
        for _, obj in ipairs(folder:GetDescendants()) do
            ...
            if obj:IsA("Model") and table.find(targetTreeNames, obj.Name) then
    ...
    print(string.format(
        "[ChopAura] Cache dibangun untuk '%s'. Total: %d pohon (%s)",
        SelectedTreeCategory,
        #TreeCache,
        table.concat(targetTreeNames, ", ")
    ))
    ...
    if #TreeCache > 0 and ChopAuraEnabled then
        WindUI:Notify({
            ...
            Content = string.format("Found %d %s trees", #TreeCache, SelectedTreeCategory),
            Duration = 4,
            Icon = "trees"
        })
    end
...
do
    ...
    -- Chop Aura Toggle
    FarmTab:Toggle({
        Title = "Chop Aura (" .. SelectedTreeCategory .. ")",
        Icon = "axe",
        Default = false,
        Callback = function(state)
            if scriptDisabled then return end
            ...
            if state then 
                ...
                WindUI:Notify({
                    ...
                    Content = string.format("AKTIF untuk %s (%d trees ditemukan)", 
                        SelectedTreeCategory, #TreeCache),
                    Duration = 4,
                    Icon = "zap"
                })
            else 
    ...
    FarmTab:Dropdown({
        Title = "Tree Category",
        Description = "Pilih jenis pohon untuk Chop Aura",
        Values = {"Small Tree", "Snowy Small Tree", "Big Tree"},
        Value = "Small Tree",
        Callback = function(value)
            SelectedTreeCategory = value
            WindUI:Notify({
                Title = "Tree Category Changed",
                Content = string.format("Sekarang target: %s", value),
                Duration = 3,
                Icon = "trees"
            })
